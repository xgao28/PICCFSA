...
```{r}
#| label: fig-year
#| fig-cap: Event Type Counts Grouped by Year
#| echo: false
#| warning: false
#| message: false
...
ggplot(df_summarized, aes(x = event_date, y = count, color = event_type, group = event_type)) + 
  geom_smooth() + 
  labs(title = "Event Type Counts Grouped by Date", 
       x = "Date", 
       y = "Count", 
       color = "Event Type") +
  theme_minimal()
...
```{r}
#| label: tab_hood
#| fig-cap: A Look-up Table for Hood Number and Hood Name
#| echo: false
#| warning: false
#| message: false
...
grid.arrange(grid.table(neighborhood_table))
```
